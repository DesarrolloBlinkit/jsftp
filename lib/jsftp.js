...
Ftp.prototype._createSocket = function(port, host, firstAction) {
	...
	this.socket.on('error', this.reemit('error'));
	this.socket.on('lookup', this.reemit('lookup'));
...
Ftp.prototype.send = function(command) {
	...
	dbgCommand(command);
	/*PERMITIR EL LOGADO EN SERVIDORES FTP QUE NO LO PERMITEN CON FACILIDAD
	 * codifica los caracteter para permitir caracteres raros excepto en la descarga o subida*/
	if(!command.includes("stor") && !command.includes("retr") && !command.includes("stat"))
		this.pipeline.write(iconv.encode(command+ '\r\n', 'ISO-8859-1') );
	else
...
Ftp.prototype.execute = function(action, callback) {
	...
	if(!action.includes("dele") || 
			(action.includes("dele") && action.includes(".lock"))){
